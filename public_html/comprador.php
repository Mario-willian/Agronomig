...
if (isset($_POST['enviar'])){
    ...
    // Extrai somente os números (Para Validar o CPF)
    $cpf_numeros = preg_replace( '/[^0-9]/is', '', $cpf );

    //Recuperar arquivo da classe
    include_once ("inc/conexao.php");

    //Select para buscar se já existe o cpf
      $result_cpf_cliente = "SELECT count(cpf) FROM cliente WHERE cpf = '$cpf' AND ativo = 1";
      $resultado_cpf_cliente = mysqli_query($conn, $result_cpf_cliente);
      $row_cpf_cliente = mysqli_fetch_assoc($resultado_cpf_cliente);
    ...
    //Select para buscar se já existe o cpf
      $result_cpf_cliente_desativado = "SELECT count(cpf) FROM cliente WHERE cpf = '$cpf' AND ativo = 0";
      $resultado_cpf_cliente_desativado = mysqli_query($conn, $result_cpf_cliente_desativado);
      $row_cpf_cliente_desativado = mysqli_fetch_assoc($resultado_cpf_cliente_desativado);
    ...
    if(preg_match('/(\d)\1{10}/', $cpf_numeros) && strlen($cpf_numeros) != 11){
    ...
    else if($row_cpf_cliente['count(cpf)']>=1 && ($row_usuario_cliente['count(usuario)']>=1 || $row_usuario_vendedor['count(usuario)']>=1 || $row_usuario_adm['count(login)']>=1)){
      $erro_usuario = "Este Usuario já está Cadastrado";
        $erro_cpf = "Este CPF já está sendo utilizado";
    }
    else if($row_cpf_cliente['count(cpf)']>=1){
        $erro_cpf = "Este CPF já está sendo utilizado";
    }
      ...
      if ($row_cpf_cliente_desativado['count(cpf)']>=1) {
        $cliente->recriar_cliente();
      }else{
            ...
            <div class="row">
                <div class="col-md-12">
                    <div class="product-bit-title text-center">
                        <h2>Cadastro de Comprador <i class="fa fa-cart-plus"></i></h2>
                    </div>
                </div>
            </div>
        </div>
...
function Onlynumbers(e)
{
  ...
  if((tecla >= "97") && (tecla <= "122")){
    ...
    <div class="form-group">
        <div class="label-float">
      <input style="border-left-color: transparent;border-top-color: transparent;border-right-color: transparent; height: 45px" type="text" class="form-control" name="nome" maxlength="80" placeholder="  " required="">
    ...
    <center><b style="color: red">
      ...
      ?>
        ...
        <div class="form-group">
            <div class="label-float">
      <input style="border-left-color: transparent;border-top-color: transparent;border-right-color: transparent; height: 45px" type="text" class="form-control" name="telefone" placeholder="  " maxlength="15"  OnKeyPress="formatar('##-#####-####', this)" required="">
    ...
    <div class="form-group">
             <div class="label-float">
      <input style="border-left-color: transparent;border-top-color: transparent;border-right-color: transparent; height: 45px " type="email" class="form-control" name="email" placeholder="  " maxlength="80" required="">
    ...
    <div class="form-group">
             <div class="label-float">
      <input style="border-left-color: transparent;border-top-color: transparent;border-right-color: transparent; height: 45px" type="text" class="form-control" name="usuario" placeholder="  " maxlength="45" required="">
            <label style="margin-left: 20px;">Usuário</label>
  </div>
    </div>
    <div class="form-group">
             <div class="label-float">
      <input style="border-left-color: transparent;border-top-color: transparent;border-right-color: transparent; height: 45px" type="password" class="form-control" name="senha" placeholder="  " maxlength="80" required="">
            <label style="margin-left: 20px;">Senha</label>
  </div>
    </div>
