...
$result_estoque = "SELECT * FROM estoque_vendedor where cpf = '$cpf_user';";
$resultado_estoque = mysqli_query($conn, $result_estoque);
$row_estoque = mysqli_num_rows ($resultado_estoque);
...
<html lang="en">
  ...
  <tbody>
    ...
    <?php while ($rows_estoque = mysqli_fetch_assoc($resultado_estoque)){ ?>
    <tr>
      <th scope="row">#<?php echo $rows_estoque['codigo_produto'];?></th>
      <td><?php echo $rows_estoque['produto'];?></td>
      <td><?php echo $rows_estoque['quantidade'];?></td>
      <td><?php echo "R$ ".number_format($rows_estoque['valor'],2);?></td>
      <td style="width: 10px">
        <form action="editar_estoque.php?codigo=<?php echo $rows_estoque['codigo_produto'];?>""><button class="btn btn-primary btn-sm" title="Editar Produto"><i class=" fa fa-edit"></i></button>
      </td>
      <input class="w3-input w3-border" name="codigo" style="display:none" type="text" value="<?php echo $rows_estoque['codigo_produto'];?>" ></form>
    </tr>
    <?php } ?>
